...
public class Combination {
     ...
     */
    public double calculate(ArrayList<Integer> listInteger, String signsCombination) {
        double result = 0;
        if ((listInteger.size() - 1) == signsCombination.length()) {
            ...
            if (signsArray.length == 1) {
                if (signsArray[0] == '+') {
                    result = (double) ((double) listInteger.get(0) + (double) listInteger.get(1));
                ...
                for (int i = 0; i < signsArray.length; i++) {
                    ...
                    if (i == 0) {
                        if ((signsArray[i] == '+') | (signsArray[i] == '-')) {
                            result = (double) (listInteger.get(i));
                        // START  -  signs "*" или "/" go in a row, and then innerResult is added to result
                        } else if ((signsArray[i] == '*') | (signsArray[i] == '/')) {
                            double innerResult = (double) (listInteger.get(i));
                            int j = i;
                            do {
                                if (signsArray[j] == '*') {
                                    innerResult = (double) ((double) innerResult * (double) listInteger.get(j + 1));
                    ...
                    /* Last sign */
                        ...
                        if (signsArray[i] == '+') {
                            result += (double) (listInteger.get(i + 1));
                            if (signsArray[i - 1] == '+') {
                                result += (double) (listInteger.get(i));
                            ...
                            if ((signsArray[i - 1] == '*') | (signsArray[i - 1] == '/')) {
                                ...
                                result = (double) ((double) result + ((double) listInteger.get(i) * (double) listInteger.get(i + 1)));
                            } else if (signsArray[i - 1] == '-') {
                                result = (double) ((double) result - ((double) listInteger.get(i) * (double) listInteger.get(i + 1)));
                            }
                        } else if (signsArray[i] == '/') {
